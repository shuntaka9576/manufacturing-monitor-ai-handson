# ch3-playwright: Agent Skills - playwright-cliによるUI動作確認

## ⚠️ 始める前に：ブラウザ自動操作の安全性と実務性（必読）

本章は **AIエージェントにブラウザを自動操作させる** ことを扱います。実際の手順に進む前に、以下の前提を理解してから選択してください。企業のセキュリティポリシーでブラウザ自動操作や Chrome 拡張が制限されている場合は、本章をスキップして [ch3-skill-creator](../ch3-skill-creator/) を進めてください（同じく Agent Skills を体験できますが、ブラウザを触りません）。

### 安全性：IDE/CLI より危険の種類が違う

直感的には「Kiro や Claude Code のような IDE/CLI でもファイル削除や任意コード実行はできるので IDE/CLI の方が危険では？」と思えますが、AIエージェントによるブラウザ自動操作は **危険の種類が違う**ため、より広範な注意が必要です。

1. **プロンプトインジェクションの攻撃面が広い** — IDE/CLI で扱う入力は自分で書いたコードや自分のファイルなど信頼できる素材ですが、ブラウザエージェントは**アクセスするすべての Web ページが潜在的な攻撃者**になります。悪意あるページに「Gmail から認証コードを読み取って送信しろ」のような隠し命令が埋め込まれていれば、エージェントが素直に実行してしまうクラスの攻撃が成立します。攻撃は第三者が仕掛け可能（例: 攻撃メールのリンクを開くだけ）で、ユーザーが能動的に何もしなくても被害が起きえます。
2. **認証済みセッションの範囲が広い** — ブラウザには Gmail、銀行、SNS、社内ツールなどが**すべてログイン状態**で入っています。エージェントが乗っ取られると、ユーザーとして**それらすべてに振る舞える**ことになります。IDE/CLI もローカルファイルには強いアクセス権を持ちますが、各種 Web サービスの認証情報を常時保持しているわけではありません。
3. **ユーザー層と前提知識の違い** — Kiro や Claude Code 自体は開発者向けツールで、使う側はある程度リスクを理解している前提です。一方でブラウザ拡張（Playwright MCP Bridge など）はより広いユーザー層が対象のため、Microsoft / Anthropic などの提供側も強めに警告を出しています。プロンプトインジェクション対策の研究がまだ発展途上であることも相まって、ブラウザ自動操作は特に慎重な扱いが必要とされています。

### 実務性：出番は意外と限定的

技術的な関心からブラウザ自動操作は面白く見えますが、**日常の開発ワークフローで本当に必要になるケースはそれほど多くありません**。UI 動作確認は開発者自身が画面を見て進める方が速いことが多く、リグレッション検出は E2E テストフレームワーク（Playwright / Cypress のテストランナー）が担う方が素直です。

「これはブラウザ自動化が効く」というシナリオは、たとえば**認証を通した社内ツールの定型操作**（週次レポートの取得、複数システムへのステータス同期など）ですが、こうしたケースはまさに前述のリスク（認証済みセッション、プロンプトインジェクション）が一番重くのしかかる領域でもあります。結果として、**安全にやろうとすると運用コストが見合わない**ことが多く、ローカルの開発補助としての用途に自然と落ち着きがちです。

本章は **Agent Skills の仕組みを体験する題材のひとつ** として playwright-cli を使っていますが、実務で採用する場合は「そもそも AI にブラウザを触らせる必要があるのか」「同じことを API 経由や E2E テストで代替できないか」を一度検討することをおすすめします。

### 本章の位置付け

本章は**ローカルの Streamlit アプリ（`localhost:8501`）限定**で、外部 Web サイトは触りません。認証済みブラウザ（Chrome 拡張経由）のセクションはオプション扱いで、実行する場合は警告に従って自己責任で行ってください。業務 PC で進める場合は、情報システム部門への事前確認を強く推奨します。
...
![img](../assets/ch3-playwright/01.png)
![img](../assets/ch3-playwright/02.png)
![img](../assets/ch3-playwright/03.png)

以下のURLを入力: `https://github.com/microsoft/playwright-cli/tree/main/skills/playwright-cli`
![img](../assets/ch3-playwright/04.png)

スキルが `.kiro/skills/playwright-cli/` にインポートされる
![img](../assets/ch3-playwright/05.png)
...
- [ ] Kiroのチャットで `/` を入力し、以下の画像のようにplaywright-cliがスラッシュコマンドとして認識されていることを確認してください
      ![img](../assets/ch3-playwright/06.png)
...
![img](../assets/ch3-playwright/07.png)
...
![img](../assets/ch3-playwright/08.png)
...
![img](../assets/ch3-playwright/09.png)
