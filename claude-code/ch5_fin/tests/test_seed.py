...
def test_equipment_ids_sequential(db_conn):
    ...
    cur = db_conn.cursor()
    ids = [row[0] for row in cur.execute("SELECT equipment_id FROM equipment ORDER BY equipment_id").fetchall()]
...
def test_equipment_roundtrip(db_conn, excel_wb):
    ...
    cur = db_conn.cursor()
    db_rows = cur.execute("SELECT name, type, location, installed_on FROM equipment ORDER BY equipment_id").fetchall()

    assert len(excel_rows) == len(db_rows)
    for excel_row, db_row in zip(excel_rows, db_rows, strict=False):
...
def test_sensor_readings_roundtrip(db_conn, excel_wb):
    ...
    db_map = {
        ...
        for eq_id, ts, temp, vib, rpm, power, pressure in cur.execute(
            "SELECT equipment_id, timestamp, temperature, vibration, rpm, power_kw, pressure FROM sensor_readings"
        )
    }
