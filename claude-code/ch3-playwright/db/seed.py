...
from db.connection import connect, apply_schema, DB_PATH
...
def _extract_sensor_readings(ws, valid_eq_ids: set) -> list[tuple]:
    ...
    for row_num, row in enumerate(rows[1:], start=2):
        ...
        for field, val in [("設備ID", eq_id), ("タイムスタンプ", ts_raw), ("temperature", temperature),
                           ("vibration", vibration), ("power_kw", power_kw)]:
            if val is None:
...
def _extract_status_logs(ws, valid_eq_ids: set) -> list[tuple]:
    ...
    for row_num, row in enumerate(rows[1:], start=2):
        ...
        for field, val in [("設備ID", eq_id), ("発生日時", occurred_at_raw), ("変更前ステータス", prev_status),
                           ("変更後ステータス", new_status), ("理由", reason)]:
            if val is None:
