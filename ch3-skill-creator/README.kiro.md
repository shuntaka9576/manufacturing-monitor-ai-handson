...
- Agent Skills の仕様調査（メインセッションを汚さない問い合わせパターンを体験）
- 対話確認ツール（Kiro の `askQuestions` / Claude Code の `AskUserQuestion`）を直接呼んで UI を体感
- `skill-creator` で `daily-operations-report` スキルを対話的に作成
- 出力した日報の確認 → `skill-creator` でレビュー → Level 1/2/3 の改善を適用
- 対話確認をスキルに組み込んで実運用寄りに育てる
...
### 1.6. Agent Skills の仕様を予習する（約4分）

本題のスキル作成に入る前に、このあと使う仕様を Kiro 側で調べさせます。

> [!NOTE]
> **Claude Code 版との違い**: Claude Code には `claude-code-guide` という組み込みサブエージェントがあり、仕様調査をメインセッションから切り離して実行できます（Haiku で軽量・高速）。Kiro は 2026-04 時点で同等の built-in subagent を持たないため、本セクションでは Vibe モードのチャットで同じ情報を取得します。サブエージェントの考え方に興味がある方は [Claude Code 版 1.6](./README.claude.md#16-claude-code-guide-で-agent-skills-の仕様を調べる約4分) も一読してください。

Kiro のチャット（Vibe モード）で以下を入力します。

```text
Agent Skills の SKILL.md で指定できる YAML frontmatter 項目と、スキル内からユーザーに
選択肢を提示して対話確認する仕組み（Kiro の askQuestions / Experimental 機能、および
Claude Code の AskUserQuestion）の使い方を教えてください。
```

- このあと **2.（スキル作成）** と **4.4.（対話確認の組み込み）** で使う知識をまとめて引き出します
- Kiro のチャットログが膨らむことを体感してください。Claude Code の `claude-code-guide` では**メインセッションに本文が展開されず**同じ情報を得られます（サブエージェントの価値）

#### チェック項目

- [ ] SKILL.md frontmatter の必須（`name`, `description`）／任意項目の説明が返ってくること
- [ ] 対話確認ツールの引数構造（選択肢・見出し・質問文）の説明が返ってくること

### 1.7. 対話確認ツールを直接体験する（約3分）

1.6 で調べた対話確認ツール（Kiro の `askQuestions` / Claude Code の `AskUserQuestion` 相当）を、**まずメインセッションから直接呼ばせて UI を体感**します。4.4 でスキルに組み込ませる前に手で触るのが目的です。

Kiro のチャットで以下を入力します。

```text
askQuestions（または同等の対話確認ツール）を使って、私に次の 3 問を一度に聞いてください。
...
- Kiro のチャット UI に選択肢（チップ or ラジオボタン）が表示されるはずです
- **4.4 で skill-creator に組み込ませる動作が、今ここで手触りとして分かる**のがこのステップの狙いです

> [!NOTE]
> Kiro の `askQuestions` は Experimental 扱い（VS Code Copilot 由来）で、Kiro バージョンによっては UI が出ずテキスト回答を求められる場合があります。その場合は「対話確認の設計体験」として、Claude Code の `AskUserQuestion` と同等の使いどころを押さえる意識で進めてください。
>
> ここで選んだ回答は**後続のセクションには使いません**。section 2 では意図的に「最小の情報だけ」で skill-creator に初版を作らせます。UI を触ること自体が目的です。

#### チェック項目

- [ ] チャット欄に選択肢 UI（または代替のテキスト確認）が表示されること
- [ ] 3 問まとめて聞かれること
- [ ] 選択後、余計な実装に進まず了解の返事で止まること
...
### 4.4. 対話確認をスキルに組み込む（約6分）

ここまでで Progressive Disclosure の構造は整いました。最後に「最小の指示で動くスキル」から **「実運用で任せられる対話型スキル」** へ進化させます。

選択肢を明示する対話ツール（Claude Code の `AskUserQuestion` / Kiro の `askQuestions` 等）は、自由入力よりモデルの暴走を抑え再現性が上がるため、運用フローに組み込みたいスキルでは第一選択になります。

1.6 で調べた引数構造を頭に置きつつ、skill-creator にリファクタを依頼します。

> [!IMPORTANT]
> このフェーズも **Spec モード**（変更前に確認するモード）で実行してください。

```text
/skill-creator で .kiro/skills/daily-operations-report/ に対話確認を追加してください。
Kiro の askQuestions 相当（または Claude Code の AskUserQuestion）を使い、
以下の2シナリオに対応させます。
...
> [!NOTE]
> Kiro の `askQuestions` は Experimental 扱い（VS Code Copilot 由来、Kiro での挙動は Kiro 側のバージョンに依存）です。期待通り UI が出ない場合は、skill-creator にフォールバックとして `echo` + ユーザー入力待ちなどの簡易実装で対話確認を表現してもらうよう伝えてください。

#### チェック項目

- [ ] `SKILL.md` または `scripts/` に対話確認の呼び出しが記述されていること
- [ ] 対象日を省略すると選択肢 UI（または代替の確認）が出ること
- [ ] 既存日付を指定したときに上書き確認が出ること
...
新しい日付で実行し、Progressive Disclosure の構造 × 対話確認が合わさった**実運用寄りのスキル**になっていることを確認します。
...
- [ ] `daily-operations-report` に対話確認（askQuestions 等）が組み込まれていること
- [ ] Agent Skills の仕様調査を体験済み（Kiro のチャットログで情報取得の流れを見た）
