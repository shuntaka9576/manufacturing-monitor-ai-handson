...
pyproject.toml に ruff の設定が追加されています。ruff を使って既存コードの品質を改善してください。

## やること

1. `uv run ruff check .` を実行してlint問題を確認
2. `uv run ruff check --fix .` で自動修正できるものを修正
3. 自動修正できない問題は手動で修正
4. `uv run ruff format .` でコードをフォーマット
5. 全ての修正後、`uv run pytest -v` でテストが通ることを確認
```
