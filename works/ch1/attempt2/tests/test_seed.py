...
def test_record_counts(db_conn):
    ...
    assert cur.execute("SELECT COUNT(*) FROM status_history").fetchone()[0] == 59
    assert cur.execute("SELECT COUNT(*) FROM sensor_data").fetchone()[0] == 1152
...
def test_cnc_rpm_not_null(db_conn):
    ...
    null_count = cur.execute(
        "SELECT COUNT(*) FROM sensor_data WHERE equipment_id IN (1, 2) AND rpm IS NULL"
    ).fetchone()[0]
    assert null_count == 0


# --- タスク 4.3: ラウンドトリップ整合性テスト ---
...
def test_equipment_roundtrip(db_conn, excel_wb):
    ...
    for idx, row in enumerate(ws.iter_rows(min_row=2, values_only=True)):
        ...
        excel_rows.append((idx + 1, name, type_, location, installation_date))

    cur = db_conn.cursor()
    db_rows = cur.execute(
        "SELECT id, name, type, location, installation_date FROM equipment ORDER BY id"
    ).fetchall()
...
def test_status_history_roundtrip(db_conn, excel_wb):
    """Excel のステータス変更履歴全行と status_history テーブル全行が一致することを確認する。"""
    ...
    cur = db_conn.cursor()
    db_rows = cur.execute(
        "SELECT equipment_id, equipment_name, occurred_at, status_before, status_after, reason "
...
def test_sensor_data_roundtrip(db_conn, excel_wb):
    """Excel のセンサーデータ全行と sensor_data テーブル全行が一致することを確認する（NaN は NULL として比較）。"""
    import math
