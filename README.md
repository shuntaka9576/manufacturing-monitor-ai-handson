...
| ch3-skill-creator     | Agent Skills（skill-creator, 安全系） | 約30分   |
...
### ch3-skill-creator: Agent Skills - skill-creator によるスキル開発（所要時間: 約30分）

**AI手法**: Agent Skills（`vercel-labs/skills` CLI + Anthropic 公式 `skill-creator`）

- [Kiro 版](./ch3-skill-creator/README.kiro.md)
- [Claude Code 版](./ch3-skill-creator/README.claude.md)

pnpm + `minimum-release-age=30240`（21日）で `skills` CLI をセキュアに固定導入。`anthropics/skills` から `skill-creator` を取り込み、設備稼働日報スキル `daily-operations-report` を対話で作成・Progressive Disclosure で改善します。ブラウザ自動操作を避けたい環境向けの ch3 代替コースです。
